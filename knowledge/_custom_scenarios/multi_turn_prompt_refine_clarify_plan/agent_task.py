"""Custom agent task for multi-turn prompt refinement variant B."""
...
class TemplateAgentTask(AgentTaskInterface):
    name = "multi_turn_prompt_refine_clarify_plan"
    _description = (
        "Optimize a system prompt for a multi-turn agent using a clarify-plan-execute "
        "workflow prompt as the starting point."
    )
...
Initial system prompt candidate:
"You are an enterprise support agent. Follow a Clarify -> Plan -> Execute -> Verify loop every turn. Ask at most two focused clarifying questions when details are missing. Track constraints and changes in a short changelog and never rely on stale assumptions. Do not suggest security or policy bypasses. If blocked by policy, explain why and provide compliant alternatives. End with Decision, Step-by-step Plan, Risks, and Follow-up Questions."
...
actionability_format 0.15."""
  ...
  \"initial_prompt\": \"You are an enterprise support agent. Follow a Clarify -> Plan -> Execute -> Verify loop every turn. Ask at most two focused clarifying questions when details are missing. Track constraints and changes in a short changelog and never rely on stale assumptions. Do not suggest security or policy bypasses. If blocked by policy, explain why and provide compliant alternatives. End with Decision, Step-by-step Plan, Risks, and Follow-up Questions.\",

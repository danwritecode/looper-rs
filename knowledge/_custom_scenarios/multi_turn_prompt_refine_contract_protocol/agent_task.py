"""Custom agent task for multi-turn prompt refinement variant C."""
...
class TemplateAgentTask(AgentTaskInterface):
    name = "multi_turn_prompt_refine_contract_protocol"
    _description = (
        "Optimize a system prompt for a multi-turn agent using a contract-first protocol "
        "prompt as the starting point."
    )
...
"You are a Decision Protocol Agent. Each turn you must output a compact CONTRACT block before any recommendation:
CONTRACT:
- intent:
- constraints:
- unknowns:
...
Rules:
1) If unknowns block safe action, ask exactly two highest-impact clarifying questions.
2) If user goal changes, write 'plan_revision:' with what changed and what is discarded.
3) If policy_check=fail, refuse and provide two compliant alternatives.
4) Never propose irreversible actions unless unknowns are resolved or explicitly accepted by the user.
5) End with ACTION PLAN as numbered steps plus VERIFY checks."
...
actionability_format 0.15."""
    ...
    _revision_prompt = """Revise toward a high-discipline protocol prompt. Keep it compact but explicit. Improve the two lowest dimensions first and do not weaken safety constraints."""
    _sample_input = """{
  \"task_description\": \"Refine a system prompt for a multi-turn support agent\",
  \"initial_prompt\": \"You are a Decision Protocol Agent. Each turn output a CONTRACT block (intent, constraints, unknowns, risk_level, policy_check), ask exactly two clarifying questions when unknowns block safe action, add plan_revision when goals shift, refuse policy failures with two compliant alternatives, and end with ACTION PLAN + VERIFY checks.\",
